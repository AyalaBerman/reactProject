...
const ServicesDetails = observer(() => {
    ...
    const dialog = () => (
        <Dialog open={toAdd} onClose={handleClose} sx={{ minWidth: 275, textAlign: 'center', alignContent: 'cenetr', alignItems: "center", direction: 'rtl' }}>
            <DialogTitle sx={{color:"orange"}}>הוספת שירות</DialogTitle>
            <DialogContent >
                <DialogContentText>
                ...
                <form onSubmit={handleSubmit(saveDetails)} sx={{ direction: 'rtl' }}>
                    ...
                    <DialogActions sx={{ minWidth: 275, textAlign: 'center', alignContent: 'cenetr', alignItems: "center" }}>
                        <Button size="medium"  variant="contained" sx={{ backgroundColor: 'orange', margin:"1em" }} onClick={handleClose}>ביטול</Button>
                        <Button type='submit' size="medium" variant="contained" sx={{ backgroundColor: 'orange', margin:"1em" }}>שמירה</Button>
                    </DialogActions>
                </form>
            </DialogContent>
        </Dialog >)
        ...
        <>
            ...
            <Grid
                ...
                alignContent="center" ><List sx={{ width: '100%', maxWidth: 600, bgcolor: 'background.paper', direction: 'rtl', textAlign: 'center' }}>
                    ...
                    )}<ListItem container item xs={2}> {useContext(ModeContext).isAdmin && <Button sx={{color:'gray'}} onClick={() => { setToAdd(true) }}><AddIcon></AddIcon></Button>}</ListItem></List>
            </Grid>
            {toAdd && dialog()}
        </>
    )
