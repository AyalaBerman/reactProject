...
export default function (details) {
    ...
    return (
        <>
            <Accordion sx={{ width: '100%', backgroundColor:'orange' }}>
                <AccordionSummary
                    expandIcon={<ExpandMoreIcon />}
                    aria-controls="panel1a-content"
                    id="panel1a-header"
                >
                    <Typography sx={{ textAlign: 'center' }}>{(!useContext(ModeContext).isAdmin) && <Button sx={{color:'gray'}} onClick={() => openDialog(details)}><CalendarMonthIcon></CalendarMonthIcon></Button>} {details.name}</Typography>
                </AccordionSummary>
